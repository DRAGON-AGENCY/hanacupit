...
xlsx 編集（openpyxl スクリプト等）や調査・検証のために作成した**一時的な作業用
ファイル**（Python/PowerShell スクリプト、中間出力、検証用のログ・CSV 等）は、
作業完了後にその場で削除し、リポジトリや作業フォルダ（`C:\work\...` 配下含む）に
残さないこと。

- 対象は「実行して終わりの使い捨てファイル」全般であり、Excel 編集用スクリプトに
  限らない（データ確認用の一時 SQL 出力、動作確認用の使い捨てスクリプト等も同様）。
- 成果物そのもの（編集後の xlsx・作成した資料本体）や、成果物の**バックアップ**
  （上書き前に取得したコピー等、ユーザーが復元に使う可能性があるもの）は対象外。
  削除してよいか判断に迷う場合は、削除前にユーザーに確認すること。
- スクラッチパッドディレクトリ（セッションごとの一時領域）に置いたファイルも、
  タスク完了時点で自分から片付ける。放置してユーザーに指摘される前に、作業完了の
  一環として削除まで行うこと。
- 過去のセッションが片付け忘れた一時ファイルを見つけた場合（例：Windowsの
  `%TEMP%`直下に残る`hanacupit_`接頭辞のファイルや、`compile_out`・`verify_out`・
  `writer_test_out`のようなjavac/java直接実行の作業用出力フォルダ、`xlsx_extract`
  等の一時展開フォルダ）も、今回作った物かどうかによらず、気づいた時点でユーザーに
  報告し、承諾を得たうえで削除する（2026-08-05に実施）。存在に気づいたまま放置しない。

---

## ネットワーク共有フォルダへのアクセス厳禁

`\\192.168.1.247\ppddev\...`（成果物格納用のネットワーク共有）へのアクセスは、
読み取り・書き込みを問わず**厳禁**とする。

- 成果物（結合テスト仕様書等のxlsx・sql等）の作成・更新は、ローカルの対応フォルダ
  （例：`C:\work\20260401_花キューピット\11_結合テスト仕様書\`）に対してのみ行う。
- ネットワーク共有側への反映（コピー・配置）は必ずユーザー自身が行う。ユーザーから
  明示的に依頼された場合であっても、Claude側からネットワーク共有パスへの
  コピー・書き込み・削除・読み取りは行わないこと。
- 内容確認のための参照読み取り（既存ファイルの中身を見るだけの操作）も対象に含む。
  ローカルに同名ファイルがある場合はローカル側のみを参照する。
