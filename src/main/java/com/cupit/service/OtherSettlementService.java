...
/**
 * その他精算データ作成（stera terminal）のビジネスロジックを担うサービス。
 * 決済種類に応じた INPUTファイルのフォーマット検証とデータ登録を行う。
 * {@link JftdSettlementService}（JFTD精算データ作成／PAYGATE Station側）と同じ処理
 * だが、対象の決済種類（stera JCB・stera code・steraクレジット）が異なる。
 * m_import_batchはPAYGATE Station側と共用し、payment_type列の値で系統を分離する。
 */
@Service
public class OtherSettlementService {

    private final FileImporterFactory fileImporterFactory;
    private final ImportBatchRepository importBatchRepository;

    public OtherSettlementService(
     ...
     * @param file        アップロードファイル
     * @param paymentType 決済種類の表示名
     * @param cutoffDate  締め日（yyyy-MM-dd）。stera terminal側は精算ファイルの日付列だけ
