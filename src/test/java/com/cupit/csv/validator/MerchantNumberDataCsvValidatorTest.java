...
/**
 * {@link MerchantNumberDataCsvValidator} のテスト。26列固定チェック、取引コード必須チェック、
 * 列数不正行の検出、取引コード重複の許容（複数行）を検証する。
 * 取引コード以外の全項目は任意のため、それらのチェックは行わない。
 */
class MerchantNumberDataCsvValidatorTest {

    private static final int COLUMN_COUNT = 26;

    private final MerchantNumberDataCsvValidator validator = new MerchantNumberDataCsvValidator();

    @Test
    void rejectsNonCsvExtension() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "merchant_number_data.txt", "text/plain", new byte[0]);
    ...
    void rejectsEmptyFile() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "merchant_number_data.csv", "text/csv", new byte[0]);
    ...
    @Test
    void allowsDuplicateTradeCodeWithinFile() throws Exception {
