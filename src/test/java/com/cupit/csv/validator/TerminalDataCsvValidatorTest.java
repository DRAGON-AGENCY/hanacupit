...
/**
 * {@link TerminalDataCsvValidator} のテスト。95列固定チェック、取引コード必須チェック、
 * 列数不正行の検出、取引コード重複の許容（複数行）を検証する。
 * 取引コード以外の全項目は任意のため、それらのチェックは行わない。
 */
class TerminalDataCsvValidatorTest {

    private static final int COLUMN_COUNT = 95;

    private final TerminalDataCsvValidator validator = new TerminalDataCsvValidator();

    @Test
    void rejectsNonCsvExtension() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "terminal_data.txt", "text/plain", new byte[0]);
    ...
    void rejectsEmptyFile() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "terminal_data.csv", "text/csv", new byte[0]);
